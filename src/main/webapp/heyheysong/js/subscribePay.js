...
function doInterval1(action) {
  ...
  if(parseInt($('#subPriceResult').text().replace(",",""))-parseInt($('#usePointResult').text().replace(",",""))-parseInt($('#coupon_result').text().replace(",",""))-subPrice<0){
    let lose_point = $( '#usePoint' ).val();
    if(Number(lose_point)+parseInt($('#subPriceResult').text().replace(",",""))-parseInt($('#usePointResult').text().replace(",",""))-parseInt($('#coupon_result').text().replace(",",""))-subPrice>0){
      lose_point = Number(lose_point)+(parseInt($('#subPriceResult').text().replace(",",""))-parseInt($('#usePointResult').text().replace(",",""))-parseInt($('#coupon_result').text().replace(",",""))-subPrice);
...
//포인트 숫자만 입력하게 합시다 시작
$('#usePoint').on("propertychange change paste input keyUp keyDown", function () {
  $('#usePoint').val($('#usePoint').val().replace(/[^0-9]/gi, ""));
})
//포인트 숫자만 입력하게 합시다 끝
...
$("#usePoint").on("propertychange change paste input keyUp", function() {
  ...
  let nonoover = parseInt($('#subPriceResult').text().replace(",",""));
  let nooover = parseInt($('#coupon_result').text().replace(",",""));
...
// 합계 구합시다 시작
function doSumsum() {
  let itsSum = parseInt($('#subPriceResult').text().replace(",",""))-parseInt($('#usePointResult').text().replace(",",""))-parseInt($('#coupon_result').text().replace(",",""));
  if(itsSum<1)
  itsSum = 0;
